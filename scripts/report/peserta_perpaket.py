...
def ConstructReportHeader(config, strTanggalAwal, strTanggalAkhir, oFile):
  ...
  oFile.write('\n')
  oFile.write('<head>\n')
  oFile.write('<title>Laporan Peserta Perpaket Aktif</title>\n')
  oFile.write('\n')
  ...
  oFile.write('		<p align="center"><font size="3" color="#008000"><b>PESERTA PERPAKET </b></font></td>\n')
  oFile.write('	</tr>\n')
  oFile.write('	<tr>\n')
...
def strSQLDaftarPeserta(config, tgl_transaksi_awal, tgl_transaksi_akhir_plus):
  return \
    'select kode_paket_investasi, count(n.no_peserta) as jmlPeserta '\
    'from NasabahDPLK n '\
    '  , RekeningDPLK r '\
    'where n.no_peserta = r.no_peserta '\
    '  and r.STATUS_DPLK IN  \'A\' '\
...
def strSQLDaftarPesertaAktif(config, tgl_transaksi_awal, tgl_transaksi_akhir_plus):
  return \
    'select kode_paket_investasi,count(n.no_peserta) as jmlPeserta '\
    'from NasabahDPLK n '\
    '  , RekeningDPLK r '\
...
def strSQLDaftarPesertaSuspend(config, kode_paket_investasi, tgl_transaksi_awal, tgl_transaksi_akhir_plus):
  return \
    'select kode_paket_investasi, count(n.no_peserta) as jmlPeserta '\
    'from NasabahDPLK n '\
    '  , RekeningDPLK r '\
...
def ConstructReportValues(config, strSQL, tgl_transaksi_awal, tgl_transaksi_akhir_plus, oFile):
  rSQLAktif = config.CreateSQL(strSQL).RawResult
  ...
  rSQLAktif.First()
  while not rSQLAktif.Eof:


    strSuspend  = strSQLDaftarPesertaSuspend(config,rSQLAktif.kode_paket_investasi ,tgl_transaksi_awal, tgl_transaksi_akhir_plus)
    ...
    jmlPeserta = (rSQLAktif.jmlPeserta + vSuspend)
    ...
    oFile.write('		<p align="center">'+ rSQLAktif.kode_paket_investasi +'</td>\n')
    ...
    oFile.write('		<p align="right">'+ str(rSQLAktif.jmlPeserta) +'</td>\n')
    oFile.write('		<td width="18%" style="border-left-style:solid; border-left-width:1px; border-right-style:solid; border-right-width:1px; border-top-style:solid; border-top-width:1px" bgcolor="'+ bgcolor +'" height="22">\n')
    oFile.write('		<p align="right">'+ str(rSQLSuspend.jmlPeserta) +'</td>\n')
    oFile.write('		<td width="18%" style="border-left-style:solid; border-left-width:1px; border-right-style:solid; border-right-width:1px; border-top-style:solid; border-top-width:1px" bgcolor="'+ bgcolor +'" height="22">\n')
...
def ConstructReportTrailer(config, totPeserta, oFile):
  ...
  oFile.write('		<b>'+ str(totPeserta) +'</b></td>\n')
  oFile.write('	</tr>\n')
  oFile.write('</table>\n')
  oFile.write('<p>&nbsp;</p>\n')
...
def WriteToFile(config, parameter, oFile):
  ...
  strSQL = strSQLDaftarPesertaAktif(config, tgl_transaksi_awal, tgl_transaksi_akhir + 1)
...
def CreateReport(config, parameter, returnpacket):
  sBaseFileName = 'peserta_perpaket.htm'
