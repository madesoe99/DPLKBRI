...
def GetValuesFromKlasifikasi(config, klasifikasi):
  if klasifikasi == 'J':
    ...
    name = 'Nama Kelompok'
    strSQL = \
      'select k.kode_nasabah_corporate kode, k.nama_kelompok nama, count(n.no_peserta) jumlah '\
    ...
    name = 'Keterangan'
    strSQL = \
      'select r.status_dplk kode, count(n.no_peserta) jumlah '\
...
def ConstructReportValues(config, strSQL, valKlasifikasi, oFile):
  ...
  while not resSQL.Eof:
    ...
    if info == 'STATUS KEPESERTAAN':
       ...
       oFile.write('		<td width="50%" style="border-left-style:solid; border-left-width:1px; border-right-style:solid; border-right-width:1px; border-top-style:solid; border-top-width:1px" bgcolor="'+ bgcolor +'">'+ nama +'</td>\n')
...
def WriteToFile(config, parameter, oFile):
  ...
  total = ConstructReportValues(config, valKlasifikasi[3], valKlasifikasi, oFile)
