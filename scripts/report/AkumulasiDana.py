...
def ConstructReportHeader(config, strTanggalAwal, strTanggalAkhir, keterangan, oFile):
  oFile.write('<html>\n')
  oFile.write('\n')
  oFile.write('<head>\n')
  ...
  oFile.write('	<tr>\n')
  oFile.write('		<td colspan="7" height="20" style="border-left:medium none #000000; border-right:medium none #000000; border-top-style:none; border-top-width:medium; border-bottom-style:none; border-bottom-width:medium">\n')
  oFile.write('		<p align="center"><font size="3" color="#008000"><b>'+ keterangan +'</b></font></td>\n')
  oFile.write('	</tr>\n')
  oFile.write('	<tr>\n')
...
def GetStatusDPLK(config, status_dplk):

    if status_dplk == 'A':
       info = 'PESERTA AKTIF'

    if status_dplk == 'N':
       info = 'PESERTA NON AKTIF'

    if status_dplk == 'S':
       info = 'PESERTA SUSPEND'

    return [info]

def strSQLSumValues(config, tgl_transaksi_awal, tgl_transaksi_akhir_plus, status_dplk):
   return \
...
def WriteToFile(config, parameter, oFile):
  ...
  status_dplk = parameter.FirstRecord.status_dplk

  info = GetStatusDPLK(config, status_dplk)
  keterangan = info[0]
  
  tgl_transaksi_awal = parameter.FirstRecord.tanggal_awal
  tgl_transaksi_akhir = parameter.FirstRecord.tanggal_akhir
  
  config.SendDebugMsg(keterangan)
  ...
  ConstructReportHeader(config, strTanggalAwal, strTanggalAkhir, keterangan, oFile)

  strSQL = strSQLSumValues(config, tgl_transaksi_awal, tgl_transaksi_akhir + 1, status_dplk)
