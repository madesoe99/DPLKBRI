...
#   sSQL = 'select r.no_peserta, r.kode_paket_investasi, t.TGL_TRANSAKSI, \
#           	t.MUTASI_IURAN_PK, t.MUTASI_IURAN_PST, t.MUTASI_PENGEMBANGAN, \
#           	t.MUTASI_PERALIHAN \
#         	from TRANSAKSIDPLK t, NASABAHDPLK n, REKENINGDPLK r \
#         	where t.ISCOMMITTED = \'T\' \
#         		and STATUS_DPLK = \'A\' \
#         	  and r.no_peserta = n.no_peserta \
#         	  and r.no_peserta = t.no_peserta \
#         		and t.TGL_TRANSAKSI > \'%s\' \
#         		and t.TGL_TRANSAKSI < DateAdd(d,1,\'%s\') \
#           order by r.kode_paket_investasi, r.no_peserta; ' \
...
#   sSQL = 'select r.no_peserta, t.kode_paket_investasi, t.TGL_TRANSAKSI, \
#           	t.MUTASI_IURAN_PK, t.MUTASI_IURAN_PST, t.MUTASI_PENGEMBANGAN, \
#           	t.MUTASI_PERALIHAN, t.id_transaksi \
#           from \
#           	(select r.no_peserta \
#             from NASABAHDPLK n, REKENINGDPLK r \
...
#           	where ISCOMMITTED = \'T\' \
#           		and TGL_OTORISASI >= \'%s\' \
#           		and TGL_OTORISASI < DateAdd(d,1,\'%s\') \
#           	) t on r.no_peserta = t.no_peserta \
#           order by t.kode_paket_investasi, r.no_peserta; ' \
#           % ('%d-%d-%d' % (tglAkhir[0],tglAkhir[1],tglAkhir[2])
...
def Main(config, tglAwalProses, tglAwal, tglAkhir, needSaving, fileLog, monfilename):
  countSleepSend = 0
  ...
  try:
    ...
    while not stop:
      ...
      if countSleepSend == 10 :
        app.ConWriteln('Proses:'+rProses.no_peserta,monfilename)
        countSleepSend = 0
      rProses.Next()
      # cek eof
      if rProses.Eof:
...
#   import rpdb2; rpdb2.start_embedded_debugger("000")
  ...
  msgProgress = sJobName + ': mulai berlangsung\r\n' 
  app.ConWriteln(msgProgress, monfilename)
  fileLog.write(msgProgress)
  try:
    #app.CreateConsole(consoleID, 'progress')
    app.ConWriteln('progress', monfilename)
    try:
      ...
      Main(config, tglAwalProses, tglAwal, tglAkhir, needSaving, fileLog, monfilename)

      #app.WriteConsole(sJobName + ': telah selesai\r\n')
      app.ConWriteln(sJobName + ': telah selesai\r\n', monfilename)
    finally:
      #app.CloseConsole(consoleID)
      fileLog.close()
  except:
    #app.WriteConsole(sJobName + ': Error\r\n' + str(sys.exc_info()[1]) + '\r\n')
    fileLog.write(sJobName + ': Error\r\n' + str(sys.exc_info()[1]) + '\r\n')
