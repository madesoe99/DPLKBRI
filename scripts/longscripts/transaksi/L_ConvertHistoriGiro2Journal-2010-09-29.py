...
def CreatePairOfRecord(config, dataset, rSQL, oMG):
  ...
  dictKet2 = {1:'dari aplikasi DPLK / Biller', 0:'dari input manual KIBLAT'}
  ...
  CreateSingleRecord(config, dataset, rSQL, 'Giro', rSQL.ACC_GIRO, ket, origin_batch, rSQL.sum_nominal_giro, 0.0)
  if rSQL.isTransaksiCreated == 'F':
    CreateSingleRecord(config, dataset, rSQL, 'Giro', oMG.acc_histori_giro, ket2, origin_batch, 0.0, rSQL.sum_nominal_giro)
  else:
    if rSQL.ACC_GIRO == '11206':
       CreateSingleRecord(config, dataset, rSQL, 'GLInterface', 'BBN_DAFTAR', ket2, origin_batch, 0.0, rSQL.sum_nominal_giro)
    else:
      if rSQL.ACC_GIRO == '11215':
       CreateSingleRecord(config, dataset, rSQL, 'GLInterface', 'KW_PREMI', ket2, origin_batch, 0.0, rSQL.sum_nominal_giro)
      else: 
       CreateSingleRecord(config, dataset, rSQL, 'GLInterface', 'KW_IURAN', ket2, origin_batch, 0.0, rSQL.sum_nominal_giro)
