...
#   elif jenisTransaksi == 'TransLRInvestasi':
  elif jenisTransaksi == 'BagiHasilDeposito':
    sSQLDepo = "select id_investasi from deposito where rekening_deposito = '%s'" % (rSQL.NOMOR_PESERTA)
    config.SendDebugMsg('sSQLDepo=' + sSQLDepo)
    rSQLDepo = config.CreateSQL(sSQLDepo).RawResult
    if not rSQLDepo.Eof:
      oDeposito = config.CreatePObjImplProxy('Deposito')
      oDeposito.Key = rSQLDepo.id_investasi
    
      o.id_investasi =  oDeposito.id_investasi
      o.no_bilyet = oDeposito.no_bilyet
      o.nama_investasi = oDeposito.rekening_deposito
...
#   if jenisTransaksi <> 'TransLRInvestasi':
  ...
  #prosesi otorisasi
  AuthorizeTransaksi.AuthorizeOperationNonTrans(config, jenisTransaksi, id, 'A')
