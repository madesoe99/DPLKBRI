...
def SavePacket(config, conn, rHistori, oHistoriGiroHarian, acc_giro):
  ...
  while not rHistori.Eof():
    trace.udp_trace(str(rHistori.GetFieldValue('Id_Detil_Transaksi')))
...
#     oHG.Nomor_Urut = rHistori.GetFieldValue('Id_Detil_Transaksi') or ''
    ...
    oHG.Transaksi_Peserta = AssignTransaksiPeserta(conn, rHistori, noref, acc_giro)
    
    ### Tambahan By Ade 2011-12-19
    #if oHG.Transaksi_Peserta == 2:
    ...
    oHG.Keterangan = ket    
    stop = 0; nomor = ''
    if acc_giro <> ACC_GIROBAGHAS:
      criteria = '[0-9,.]+'
    else:
      criteria = '[0-9]+'
      
    if re.search(criteria, ket):
      r = re.search(criteria, ket)		
      nomor = r.group()
    else:
      stop = 1      
...
def IsReversal(conn, rHistori):
    id_transaksi = rHistori.GetFieldValue('Id_Transaksi')
    sSQLa = "SELECT 1 FROM REVERSETRANSACTION \
    WHERE id_transaksi = %s" % id_transaksi
    rSQLa = conn.GetQuery(sSQLa)
    sSQLb = "SELECT 1 FROM EXTTRANSPROCESS \
    WHERE id_transaksi = %s and process_status = 2" % id_transaksi
    rSQLb = conn.GetQuery(sSQLb)
    
    return not rSQLa.Eof() or not rSQLb.Eof()   

def AssignTransaksiPeserta(conn, rHistori, noref, acc_giro):
  # untuk mengexclude transaksi yg direversal dr T2KR liability maupun accounting

  if IsReversal(conn, rHistori): Transaksi_Peserta = 2
  else:
    if acc_giro == ACC_GIROBAGHAS:
      Transaksi_Peserta = 0
    else:  
      ...
      if rHistori.GetFieldValue('IsBillerTransaction') == 'T':
        Transaksi_Peserta = 1 #2
      elif (noref[0:3]=='DP_'):
        Transaksi_Peserta = 1
      else:
        Transaksi_Peserta = 0
          
  return Transaksi_Peserta
