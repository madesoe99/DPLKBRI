...
def HitungBiaya(config, classJenisBiaya, rSQL, kode_jenis_transaksi, tglHitung, FixValue, TotalDana, blnProses):
  if classJenisBiaya == 'BiayaPengelolaanDana':
    #tentukan proporsi biaya
    proporsiBiaya = transaksiapi.HitungProporsiBiayaBulanan(config, kode_jenis_transaksi, \
...
def Main(config, tglHitung, idBatch, classJenisBiaya, blnProses):
  ...
  try:  
    ...
    try:
      while not rSQL.Eof:
        ...
        besarBiaya = HitungBiaya(config, classJenisBiaya, rSQL, kode_jenis_transaksi, tglHitung, FixValue, TotalDana, blnProses)
...
def DAFLongScriptMain(config, parameter, pid, monfilename):
  ...
  try:
    ...
    try:
      ...
      Main(config, tglHitung, idBatch, classJenisBiaya, blnProses)
