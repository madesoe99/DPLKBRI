...
def HitungBiaya(config, classJenisBiaya, rSQL, kode_jenis_transaksi, tglHitung, FixValue, TotalDana):
  ...
  if classJenisBiaya == 'BiayaPengelolaanDana':
    ...
    if proporsi > 1:
       besarBiaya = proporsiBiaya * 1500  #FixValue
    else:
...
def GetParameter(config, classJenisBiaya, SRR):
  ...
  if classJenisBiaya == 'BiayaAdmTahunan':
    ...
    if SRR <= batas_srr:
      ...
      oP.Key = 'PERSEN_BIAYA_PENGELOLAAN_1' ; FixValue = (oP.Numeric_Value/12)/ 100
    else:
      #config.SendDebugMsg('> 100jt')
      oP.Key = 'PERSEN_BIAYA_PENGELOLAAN_2' ; FixValue = (oP.Numeric_Value/12)/ 100
...
def CreateBiayaTahunan(config, classJenisBiaya, tglHitung, noPeserta, valueBiaya):
...
def Main(config, tglHitung, classJenisBiaya):
  ...
  try:  
    ...
    try:
      while not rSQL.Eof:
        ...
        CreateBiayaTahunan(config, classJenisBiaya, tglHitung, rSQL.no_peserta, besarBiaya)
        #config.FlushUpdates()        
        i += 1
        rSQL.Next()
    finally:
...
def DAFLongScriptMain(config, parameter, pid, monfilename):
  ...
  try:
    ...
    try:
      ...
      Main(config, tglHitung, classJenisBiaya)
