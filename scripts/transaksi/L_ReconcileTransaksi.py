...
def HapusTransaksiNonCommit(config, tTglTransaksi):
  ...
  for transaksi in listTransaksi:
    ...
    #backup terlebih dahulu transaksi yang akan dihapus ke tabel temporer
    BackupDeletedTransaction(config,transaksi,y,m,d)
    config.FlushUpdates()
...
def BackupDeletedTransaction(config,transaksi,y,m,d):
    sBackup = 'insert into TRANSDPLK_REKONDELETED \
               select t.* \
                  from %s as t, TransactionBatch as tb \
                  where tb.tgl_used = \'%s\' and \
                        tb.batch_type = \'%s\' and \
