...
def strSQLHistHslInv(config, id_investasi, tgl_transaksi_awal, tgl_transaksi_akhir_plus):
  return \
    '  Select clsfTransaksiInvestasi as clsfTransaksiInvestasi '\
...
def strSQLDepositoAktif(config, tgl_transaksi_akhir_plus ):
  return \
...
def strSQLDepositoTutup(config, tgl_transaksi_awal, tgl_transaksi_akhir_plus ):
  return \
    'select d.id_investasi '\
    ' ,d.rekening_deposito '\
    ' , i.no_bilyet '\
...
def ConstructReportValues(config, strSQL, tgl_transaksi_awal, tgl_transaksi_akhir_plus, flagStat, oFile):
  moduleapi = modman.getModule(config, 'moduleapi')
  
  resSQL = config.CreateSQL(strSQL).RawResult
  config.SendDebugMsg('masukkkkk.......')
  no_urut = 0
  nomHasilreal = 0.0
  nomHasilNonreal = 0.0
  nomBeban = 0.0
  ...
  while not resSQL.Eof:
   ...
   ## Status Aktif
   if flagStat == 'A' or flagStat == 'N':
   
      strSQLRincian = strSQLHistHslInv(config, resSQL.id_investasi, tgl_transaksi_awal, tgl_transaksi_akhir_plus)
      rSQLRincian = config.CreateSQL(strSQLRincian).RawResult
      ...
      if rSQLRincian.eof:
         ...
         if resSQL.tgl_jatuh_tempo <> None:
            ...
            strTglJatuhTempo = config.FormatDateTime('dd/mmm/yyyy',moduleapi.DateTimeTupleToFloat(config, resSQL.tgl_jatuh_tempo))

            if depo == 'Berjangka':
               #config.SendDebugMsg('No Bilyet : '+ resSQL.no_bilyet)
               #config.SendDebugMsg('Kode Pihakk Ke III : '+ resSQL.kode_pihak_ketiga)
               #config.SendDebugMsg('invManual-01A')
               y,m,d = (resSQL.tgl_jatuh_tempo)[:3]
               #config.SendDebugMsg('invManual-01A1 '+str(y) +' '+ str(m)+' '+ str(d))
               strTanggalAwal = config.ModLibUtils.DecodeDate(tgl_transaksi_awal)
               yy,mm,dd = strTanggalAwal[:3]
               #config.SendDebugMsg('invManual-01B '+str(yy) +' '+ str(mm)+' '+ str(dd))
               ...
               #config.SendDebugMsg('invManual-011A : '+str(invManual))
               invManual = invManual + ((resSQL.nominal_pembukaan * (float(equivalent_rate)/100)) * (float(d)/365))
               #config.SendDebugMsg('invManual-011B : '+str(invManual))
               #config.SendDebugMsg('No Bilyet : '+ resSQL.no_bilyet)
         ...
         if rSQLRincian.clsfTransaksiInvestasi == 'B':
            # Piutang pendapatan
            #config.SendDebugMsg('masukkkkk.......A1')
            mutasi_kredit = rSQLRincian.kredit or 0.0
            mutasi_debet  = rSQLRincian.debet or 0.0
            nomHasilNonreal  = (mutasi_debet - mutasi_kredit)
            hasilinvestasi = nomHasilNonreal

         elif rSQLRincian.clsfTransaksiInvestasi == 'C':
            # Pendapatan
         ...
         while not rSQLRincian.Eof:
      
            #config.SendDebugMsg('invManual-03')
            invManual = 0.0
            ...
            strTglTransaksi = config.FormatDateTime('dd/mmm/yyyy',moduleapi.DateTimeTupleToFloat(config, rSQLRincian.tgl_transaksi))
...
def WriteToFile(config, parameter, oFile):
  ...
  #strSQL = strSQLDepositoAktif(config, tgl_transaksi_akhir + 1)
  #config.SendDebugMsg(strSQL)
  #flagAktif = 'A'

  #totHasilInvestasi = ConstructReportValues(config, strSQL, tgl_transaksi_awal, tgl_transaksi_akhir + 1, flagAktif, oFile)
  ...
  #flagTutup = 'N'
  #strSQL = strSQLDepositoTutup(config, tgl_transaksi_awal, tgl_transaksi_akhir + 1)
  #config.SendDebugMsg(strSQL)
  #totHasilInvestasi = ConstructReportValues(config, strSQL, tgl_transaksi_awal, tgl_transaksi_akhir + 1, flagTutup, oFile)
