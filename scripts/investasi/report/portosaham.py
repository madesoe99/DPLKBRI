...
def ConstructReportHeader(config, dtDate, oFile):
  ...
  oFile.write('\n')
  oFile.write('<head>\n')
  oFile.write('<title>Laporan Portofolio Investasi Saham</title>\n')
  oFile.write('\n')
...
def resSQLJenisReks(config):
  strSQL = '\
    select kode_jns_Saham, jenis_Saham \
    from JenisSaham \
    order by kode_jns_Saham'
  strSQL = 'select kode_jns_Saham, jenis_Saham \
    from JenisSaham where kode_jns_Saham = \'RDC\''
  return config.CreateSQL(strSQL).RawResult

def resSQLSaham(config, kode_jns_Saham, strNextDateSQL):
  strSQL = '\
    select \
    	nama_Saham \
    	, NAB_Transaksi \
    	, unit_penyertaan \
    	, NAB \
    from Saham r, Investasi i \
    where status = \'T\' \
    	and r.id_investasi = i.id_investasi \
    	and kode_jns_Saham = \'%s\' \
    	and tgl_buka < %s'\
    % (kode_jns_Saham, strNextDateSQL)
  ...
  strSQL = '\
    select \
    	...
    	, nama_pihak_ketiga \
    	, i.id_investasi \
    from Saham r, Investasi i,pihakketiga p \
    where status = \'T\' \
      and i.kode_pihak_ketiga = p.kode_pihak_ketiga \
    	and r.id_investasi = i.id_investasi \
    	and kode_jns_Saham in (\'RDPT\',\'RDS\',\'RDC\') \
    	and tgl_buka < %s'\
    % (strNextDateSQL)
  return config.CreateSQL(strSQL).RawResult

#Tambahan By Ade Herman 2011-04-07
def resSQLSahamTutup(config, kode_jns_Saham, strNextDateSQL):
  strSQL = '\
    select \
    	nama_Saham \
    	, NAB_Transaksi \
...
def akumDataPembelian(config,id_investasi, strNextDateSQL) :
  strSQL = '\
    ...
    from SubscribeSaham s, TransaksiInvestasi i \
  ...
  strSQL = '\
    ...
    from RedemptSaham r, TransaksiInvestasi i \
    where isCommitted in (\'T\') \
...
def NABSekarang(config, id_investasi, strNextDateSQL) :
  strSQL = '\
    ...
    where h.id_investasi = %s \
      and terminaloto <> \'1\' \
    	and tgl_penetapan < %s \
    order by HistNABSahamID desc '\
    % (id_investasi,strNextDateSQL)
  return config.CreateSQL(strSQL).RawResult


def WriteTotalJenis(config, jenis_Saham, sumUnit, sumTotalrpPer, sumTotalrpWjr, oFile):
  ...
  oFile.write('		<font size="2">Total '+ (jenis_Saham or '&nbsp;') +'</font></td>\n')
...
def ConstructReportValues(config, dtDate, oFile):
  ...
  while not resJR.Eof:
    ...
    res = resSQLSaham(config, resJR.kode_jns_Saham, strNextDateSQL)
    
    if not res.Eof:
      jenis_Saham = ''#'Reksa Dana Pasar Uang, Reksa Dana Pendapatan Tetap, Saham Saham dan Reksa Dana Campuran'
      # catat jenis Saham
      oFile.write('	<tr>\n')
      # no urut
      ...
      oFile.write('		<td width="'+ str(NAMACOL) +'" style="border-left-style:solid; border-left-width:1px; border-right-style:solid; border-right-width:1px; border-top-style:solid; border-top-width:1px" bgcolor="#FFFFFF">\n')
      #oFile.write('		<font size="2">'+ (resJR.jenis_Saham or '&nbsp;') +'</font></td>\n')
      oFile.write('		<font size="2">'+ (jenis_Saham or '&nbsp;') +'</font></td>\n')
      #oFile.write('	</tr>\n')
      ...
      while not res.Eof:
        ...
        # nama produk
        oFile.write('		<td width="'+ str(NAMACOL) +'" style="border-left-style:solid; border-left-width:1px; border-right-style:solid; border-right-width:1px; border-top-style:solid; border-top-width:1px" bgcolor="#FFFFFF">\n')
        oFile.write('		<font size="2">'+ (res.nama_Saham or '&nbsp;') +'</font></td>\n')
    ...
    #Tambahan By Ade Herman 2011-04-07
    #res = resSQLSahamTutup(config, resJR.kode_jns_Saham, strNextDateSQL)
      ...
      #WriteTotalJenis(config, resJR.jenis_Saham, sumUnit, sumTotalrpPer, sumTotalrpWjr, oFile)
      WriteTotalJenis(config, jenis_Saham, sumUnit, sumTotalrpPer, sumTotalrpWjr, oFile)
...
def DAFScriptMain(config, parameter, returnpacket):
  ...
  sBaseFileName = 'portofolio_Saham.htm'
