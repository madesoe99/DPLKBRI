...
def ConstructReportHeader(config, oFile):
  ...
  oFile.write('\n')
  oFile.write('<head>\n')
  oFile.write('<title>Bagi Hasil Deposito</title>\n')
  oFile.write('\n')
  ...
  oFile.write('		<font size="3"><b>Bagi Hasil Deposito</b></font></td>\n')
  oFile.write('	</tr>\n')
  oFile.write('	<tr>\n')
...
def DAFScriptMain(config, parameter, returnpacket):
  ...
  oBagiHasilDeposito.Key = parameter.FirstRecord.id
  ...
  ConstructReportValues(
    config
    , oBagiHasilDeposito.LDeposito.no_bilyet
    , oBagiHasilDeposito.LTransactionBatch.no_batch
    , oBagiHasilDeposito.LDeposito.LPihakKetiga.kode_pihak_ketiga
    , oBagiHasilDeposito.LDeposito.LPihakKetiga.nama_pihak_ketiga
    , oBagiHasilDeposito.LDeposito.kode_paket_investasi
    , oBagiHasilDeposito.tgl_transaksi
    , oBagiHasilDeposito.nominal
    , oFile
  )

  ConstructReportTrailer(
    config
    , oBagiHasilDeposito.user_id
    , oBagiHasilDeposito.user_id_auth
    , oFile
  )
