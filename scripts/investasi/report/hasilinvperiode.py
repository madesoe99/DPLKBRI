...
def ConstructReportHeader(config, startDate, endDate, oFile):
  ...
  oFile.write('\n')
  oFile.write('<head>\n')
  oFile.write('<title>Laporan Hasil Investasi Per Periode</title>\n')
  oFile.write('\n')
  ...
  oFile.write('		<font size="3" color="#008000"><b>Laporan Hasil Investasi Per Periode</b></font><br/></td>\n')
  oFile.write('	</tr>\n')
...
def resSQLReksadana(config, kode_jns_reksadana, strSQLStartDate, strSQLEndDate):
  ...
  strSQL = '\
    ...
    from TransaksiInvestasi t, Reksadana r \
    where isCommitted = \'T\' \
    	and clsfTransaksiInvestasi in (\'C\',\'E\') \
    	and t.id_investasi = r.id_investasi \
...
def WriteContentRow(config, strJenis, strSQLStartDate, strSQLEndDate, oFile, lsJnsReks = None):
  ...
  while not res.Eof:
    if res.clsfTransaksiInvestasi == 'B':
      # piutang pendapatan
      mutasi_debet = res.mutasi_debet or 0.0
      mutasi_kredit = res.mutasi_kredit or 0.0
      ...
      mutasi_debet = res.mutasi_debet or 0.0
      mutasi_kredit = res.mutasi_kredit or 0.0
      nomBeban += (mutasi_kredit - mutasi_debet)
    elif res.clsfTransaksiInvestasi == 'E':
      mutasi_debet = res.mutasi_debet or 0.0
      mutasi_kredit = res.mutasi_kredit or 0.0
      nomBeban += (mutasi_debet - mutasi_kredit)
    res.Next()
...
def ConstructReportTrailer(config, strSQLStartDate, strSQLEndDate, oFile):
  ...
  # BARIS TOTAL INVESTASI AWAL PERIODE
  oFile.write('	<tr>\n')
  oFile.write('		<td width="'+ str(NOCOL) +'" style="border-left:1px solid #000000; border-right-style:solid; border-right-width:1px; border-top-style:solid; border-top-width:1px; border-bottom-style:solid; border-bottom-width:1px" align="right" bgcolor="#CC6699">\n')
  oFile.write('		&nbsp;</td>\n')

  oFile.write('		<td width="'+ str(JENISCOL) +'" style="border-left:1px solid #000000; border-right-style:solid; border-right-width:1px; border-top-style:solid; border-top-width:1px; border-bottom-style:solid; border-bottom-width:1px" align="right" bgcolor="#CC6699">\n')
  oFile.write('		<p align="left"><b><font size="2">Total Investasi Awal Periode</font></b></td>\n')
  ...
  # BARIS TOTAL INVESTASI AKHIR PERIODE
  oFile.write('	<tr>\n')
  oFile.write('		<td width="'+ str(NOCOL) +'" style="border-left:1px solid #000000; border-right-style:solid; border-right-width:1px; border-top-style:solid; border-top-width:1px; border-bottom-style:solid; border-bottom-width:1px" align="right" bgcolor="#CC6699">\n')
  oFile.write('		&nbsp;</td>\n')

  oFile.write('		<td width="'+ str(JENISCOL) +'" style="border-left:1px solid #000000; border-right-style:solid; border-right-width:1px; border-top-style:solid; border-top-width:1px; border-bottom-style:solid; border-bottom-width:1px" align="right" bgcolor="#CC6699">\n')
  oFile.write('		<p align="left"><b><font size="2">Total Investasi Akhir Periode</font></b></td>\n')

  # total investasi akhir periode
  sumTotalAkhir = 0.0
...
def DAFScriptMain(config, parameter, returnpacket):
  ...
  startDate = parameter.FirstRecord.dateFrom
  endDate = parameter.FirstRecord.dateUntil
  nextYearDate = endDate + 1
   
  strSQLStartDate = config.FormatDateTimeForQuery(config.ModLibUtils.CutDate(startDate))
  strSQLEndDate = config.FormatDateTimeForQuery(config.ModLibUtils.CutDate(nextYearDate))

  sBaseFileName = 'hasil_investasi_periode.htm'
