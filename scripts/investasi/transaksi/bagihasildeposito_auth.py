...
def RolloverInvestasi(config, oTransPiutangInvestasi, tt):
  ...
  if int(JT-10) < int(tt) or oDeposito.jenisJatuhTempo in (0,1):
     pass#moduleapi.AdvanceJatuhTempo(config, oDeposito)
...
def CreateTransPiutangLRInvestasiRO(config, isKapitalisir, oDeposito, oRolloverDeposito = None):
...
def RolloverMain(config, oDeposito,  isKapitalisir, nom, tgl_transaksi):
  ...
  y,m,d = oDeposito.tgl_jatuh_tempo[:3]
  JT = config.ModDateTime.EncodeDate(y,m,d)
  config.SendDebugMsg('jty : '+ str(y))
  config.SendDebugMsg('jtm : '+ str(m))
  config.SendDebugMsg('jtd : '+ str(d))

  y,m,d = tgl_transaksi[:3]
  tt = config.ModDateTime.EncodeDate(y,m,d)
  config.SendDebugMsg('tty : '+ str(y))
  config.SendDebugMsg('ttm : '+ str(m))
  config.SendDebugMsg('ttd : '+ str(d))
