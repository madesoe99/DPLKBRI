...
##  Tambahan By Ade Herman 31-01-2011 --------#
def resSQLAkumSblmBagi(config, kode_paket_investasi):

   y,m,d = config.ModDateTime.DecodeDate(config.Now())
   tglHitung = [y,m,d]
   
   strSQL = \
         "select r.kode_paket_investasi, (sum(t.prosenKredit)-sum(t.prosenDebet)) as prosentase \
            from rincianinvestasi r, ( \
              select r.kode_paket_investasi as kode_paket_investasi, \
               ...
               where r.id_investasi = t.id_investasi \
                 and r.kode_paket_investasi = '%s' \
                 and t.tgl_transaksi >= '%d-%d-01' \
                 and t.tgl_transaksi < '%d-%d-01'  \
                 and t.kode_jenis_trinvestasi IN ('E','I','K') \
               ) t \
          where r.kode_paket_investasi = t.kode_paket_investasi \
            and r.id_investasi = t.id_investasi \
            and r.kode_paket_investasi = '%s' \
          group by r.kode_paket_investasi ; "\
          % (kode_paket_investasi,tglHitung[0],tglHitung[1],tglHitung[0],tglHitung[1]+1,kode_paket_investasi)
...
def getHasilInvBelumDibagikan(config, kode_paket_investasi):
  # Hasil investasi yang belum dibagikan
  resSQL = resSQLAkumSblmBagi(config, kode_paket_investasi)
  if resSQL.Eof:
    return 0.0
  return resSQL.prosentase

def getHasilInvBelumDibagikanNonDepo(config, kode_paket_investasi):
  # Hasil investasi yang belum dibagikan
  resSQL = resSQLAkumSblmBagiDepo(config, kode_paket_investasi)
  if resSQL.Eof:
    return 0.0
  return resSQL.prosentase
...
#   nominalGiro = 10000000000.0
  ...
  # Sementara ditutup 2011-08-18 By ade herman
  hasilInvBelumDibagikan = 0.0 #getHasilInvBelumDibagikan(config,kode_paket_investasi) or 0.0
  #  End

  ## akum sukuk / Reksadana ------#
  if  kode_jns_investasi in ('O','R'):
      kode_jns_investasi = 'D'
      dpkDiinvestasikan = getInvExisting(config, kode_paket_investasi, kode_jns_investasi) or 0.0
  else:
...
#   end of ita
  ...
  return dpkPaket, dpkInvExisting, dpkTersedia, nilaiMaksProporsi, nominalGiro  #, dpkDiinvestasikan
