...
def resSQLAkumNominal(config, kode_paket_investasi):
  ...
  if kode_paket_investasi in ('A'):
    ...
    dictMapKodeInv = {'B':'D','C':'D'}
    strSQL = "select sum(akum_nominal) as sum_nominal \
            from investasi \
            where status = 'T' \
            and kode_paket_investasi = '%s' \
            and kode_jns_investasi = '%s'" % (kode_paket_investasi,dictMapKodeInv[kode_paket_investasi])
...
#   nominalGiro = 10000000000.0
  ...
  config.SendDebugMsg('dpkPaket 1='+str(dpkPaket))
  config.SendDebugMsg('dpkDiinvestasikan 1='+str(dpkDiinvestasikan))
  config.SendDebugMsg('dpkInvExisting 1='+str(dpkInvExisting))
