...
def displayWithData(sender, context):
  ...
  formid = sender.StringTag
  aform = app.GetFormWithData('investasi/master/'+formid,formid,0,key,'uipJnsTransInvestasi')
  SetToCenterForm(context.OwnerForm, aform.FormObject)
  aform.Show()
...
def mnuDeleteClick(sender, context):
  app = context.OwnerForm.ClientApplication
  try:
    dlg = app.ConfirmDialog('Anda yakin menghapus jenis transaksi %s?' \
      % (context.GetFieldValue('JnsTransInvestasi.deskripsi')))
    if dlg:
      key = context.KeyObjConst
      app.DeletePObj(key)
      context.DeleteRow()
