...
def displayWithData(sender, context):
  ...
  formid = sender.StringTag
  aform = app.GetFormWithData('investasi/master/'+formid,formid,0,key,'uipPihakKetiga')
  SetToCenterForm(context.OwnerForm, aform.FormObject)
  aform.Show()
...
def mnuDeleteClick(sender, context):
  app = context.OwnerForm.ClientApplication
  try:
    dlg = app.ConfirmDialog('Anda yakin menghapus pihak ketiga %s?' \
      % (context.GetFieldValue('PihakKetiga.nama_pihak_ketiga')))
    if dlg:
      key = context.KeyObjConst
      app.DeletePObj(key)
      context.DeleteRow()
