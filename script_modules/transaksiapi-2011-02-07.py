...
def HitungProporsiBiaya(config, kode_jenis_transaksi, no_peserta, tgl_transaksi):
  ...
  if rSQL.Eof:
    ...
    oN.Key = no_peserta
    config.SendDebugMsg('tglawal='+str(oN.tgl_registrasi[0])+str(oN.tgl_registrasi[1])+str(oN.tgl_registrasi[2]))
    ...
    config.SendDebugMsg('tglawal='+str(rSQL.tgl_transaksi[0])+str(rSQL.tgl_transaksi[1])+str(rSQL.tgl_transaksi[2]))
...
#-------------------------------------------------------------------------------
## PERHITUNGAN PROPORSIONAL BIAYA PENGELOLAAN / BIAYA ADMINISTRASI BULANAN
#-------------------------------------------------------------------------------
