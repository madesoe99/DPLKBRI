...
def FormatFloatExtStd(config, floatVal, valIfZero = '0'):
  if IsApproxZero(floatVal):
    return valIfZero
  else:
    return config.FormatFloat('#,##0.0000',floatVal)

def FormatFloatStdO(config, floatVal, valIfZero = '0'):
  if IsApproxZero(floatVal):
    return valIfZero
  else:
    return config.FormatFloat('#,##0',floatVal)
...
def GetCounterNumber(config, id_code, param, countnum = 0):
  ...
  try:
    # Check if id_code and param is exist - added by Ramdhan
    sqlCheckFirst = '\
      ...
      WHERE id_code = \'%s\' \
        AND param = \'%s\'' \
      % (id_code, param)
    resSQL = config.CreateSQL(sqlCheckFirst).RawResult
    if resSQL.count in [None, '', 0]:
      sqlFirstData = "INSERT INTO counter (id_code, is_locked, last_id, param) \
        VALUES ('%s', %d, %d, '%s')" \
        % (id_code, 0, 1, param)
      config.ExecSQL(sqlFirstData)    
    # ----------- added by Ramdhan ----------------
    ...
    # set lock
#     config.SendDebugMsg(strSQLLock)
    i = config.ExecSQL(strSQLLock)
#     config.SendDebugMsg('ilock='+str(i))
    if i <= 0:
      raise Exception,'\n\nPERINGATAN\nPenguncian record tidak berhasil!'
    ...
    # increment counter
    config.SendDebugMsg(strSQLIncrement)
    if config.ExecSQL(strSQLIncrement) <= 0:
      raise Exception,'\n\nPERINGATAN\nPenguncian record tidak berhasil!'

    # set unlock
    if config.ExecSQL(strSQLUnlock) <= 0:
      raise Exception,'\n\nPERINGATAN\nPembukaan kunci record tidak berhasil!'
...
def GetSaldoAwal(config, no_rekening, str_dari_tanggal):
  ## menghitung saldo awal pada tanggal str_dari_tanggal,
  ## untuk rekening no_rekening, dengan cara menjumlahkan
  ## total mutasi rekening peserta tersebut hingga sebelum str_dari_tanggal,
...
def GetSaldoById(config, no_peserta, id_terakhir):
  ## menghitung saldo awal pada tanggal str_dari_tanggal,
  ...
  strSQL = "\
    ...
    sum(mutasi_iuran_pst) + sum(mutasi_iuran_pk)) as saldo_awal \
      from TransaksiRekInvDPLK \
      where no_peserta = '%s' \
      and kode_jenis_transaksi not in ('A','B') \
      and isCommitted = 'T' \
      and id_transaksi > %s " % (no_peserta, id_terakhir)
  config.SendDebugMsg('saldo awal='+strSQL)
  saldo_awal = config.CreateSQL(strSQL).RawResult.saldo_awal
  if saldo_awal <> None:
    return saldo_awal
...
def IsPesertaAktif(config, no_peserta):
  ...
  oNasabahDPLK = IsNasabahAvail(config, no_peserta)

  isActive = False
  LsRekInvDPLK = oNasabahDPLK.Ls_RekeningDPLK
  LsRekInvDPLK.First()
  while not LsRekInvDPLK.EndOfList:
    oRekInvDPLK = LsRekInvDPLK.CurrentElement
    if oRekInvDPLK.status_dplk == 'A':
      isActive = True
      break
...
def CheckCorporateEditdocUniq(config, kode_nasabah_corporate):
  strSQL = \
   'select regeditndplkcorp_id '\
   'from regeditnasabahdplkcorporate '\
...
def CheckTransactionValidity(config, oInvestasi, mode, Jns = 'B'):
  if IsInvestasiTutup(config, oInvestasi) and (mode in ['uipInvestasi', 'uipDeposito', 'uipObligasi', 'uipReksadana', 'uipSaham']) and Jns != 'I' :
    raise Exception,'\n\nPERINGATAN\nInvestasi sudah ditutup.'
  y,m,d = oInvestasi.tgl_buka[:3]
  TB = config.ModDateTime.EncodeDate(y,m,d)
  
  if TB >= int(config.Now()) and Jns != 'I' :
    pass#raise Exception,'\n\nPERINGATAN\nTransaksi tidak bisa dilakukan pada hari yang sama dgn tanggal buka'
...
def ProsesChargeBiaya(config, oBiaya, nominalBiaya):
  ...
  oBiaya.mutasi_pmb_tmb = 0.0
  oBiaya.mutasi_pmb_pst = 0.0
  oBiaya.mutasi_pmb_pk = 0.0
  oBiaya.mutasi_pmb_psl = 0.0
  ...
  loopAgain = 1
  urutanPosDana = 0
  while (nominalBiaya > zero_approx) and loopAgain:
    ...
    if urutanPosDana > len(tPosDanaBiaya)-1:
      loopAgain = 0
      if nominalBiaya > zero_approx:
        config.SendDebugMsg('Dana peserta %s tidak mencukupi untuk membayar \
          %s, masih terutang Rp %0.2f' % (oRekInv.no_peserta, oBiaya.ClassName, nominalBiaya))

    # next loop
    urutanPosDana += 1
  #--
  
  # set detil transaksi DPLK oBiaya
...
def CountUnAuthTransaksiDPLK(config, no_rekening, excludeIDTrans):
  strExcludeIDTrans = '; '
  if excludeIDTrans:
    # excludeIDTrans != None
  ...
  strSQL = \
    'SELECT COUNT(1) nbOfTrans '\
    'FROM TransaksiRekInvDPLK '\
    'WHERE '\
    '  isCommitted = \'F\' '\
    '  AND no_rekening = \'%s\' '\
    '  %s '\
    % (no_rekening, strExcludeIDTrans)
  resSQL = config.CreateSQL(strSQL).RawResult
  return resSQL.nbOfTrans

def CountUnAuthTransaksiPremi(config, no_rekening, excludeIDTrans):
  strExcludeIDTrans = '; '
  if excludeIDTrans:
    # excludeIDTrans != None
    strExcludeIDTrans = '  and ID_Transaksi <> %d ' % (excludeIDTrans)
  ...
  strSQL = \
    ...
    'from titipanpremi p '\
    'INNER JOIN TransaksiRekInvDPLK r ON p.id_transaksi = r.id_transaksi '\
    'where '\
    '  r.isCommitted = \'F\' '\
    '  and r.no_rekening = \'%s\' '\
    '  %s '\
...
def CountUnAuthIuranPendaftaran(config, no_rekening, excludeIDTrans):
  strExcludeIDTrans = '; '
  if excludeIDTrans:
    # excludeIDTrans != None
    strExcludeIDTrans = '  and ID_Transaksi <> %d ' % (excludeIDTrans)
...
def CountUnAuthTransaksiPeserta(config, no_rekening, excludeIDTrans = None):
  return \
    CountUnAuthTransaksiDPLK(config, no_rekening, excludeIDTrans)  \
    + CountUnAuthTransaksiPremi(config, no_rekening, excludeIDTrans) \
    + CountUnAuthIuranPendaftaran(config, no_rekening, excludeIDTrans)
...
def GetTotalDanaPaket(config, tgl_awal, kode_paket_investasi):
  sSQL = 'SELECT \
			...
			FROM RekeningDPLK R, TransaksiRekInvDPLK T \
			WHERE \
