...
def TotalPenarikanSebelumnya(config, tgl_transaksi, noRekening):  
  ...
  sOQL = 'select from PenarikanDana [tgl_transaksi >= \'%s\' ' \
    'and tgl_transaksi < \'%s\' and LRekeningDPLK.no_rekening = \'%s\'] ' \
    '(no_rekening, jml_tarik, self) then group by no_rekening ' \
    'columns jml_tarik sum;' % (tgl_awal, tgl_akhir, noRekening)    
...
def GetLastTglPenarikan30(config, noRekening):
  #ambil tanggal penarikan 30% terakhir
  sSQL = 'select top 1 TGL_TRANSAKSI from TRANSAKSIREKINVDPLK where NO_REKENING = \'%s\' and '\
...
def CompareLastTglPenarikan(config, tgl_transaksi):
  ...
  if (config.ModLibUtils.Now() - tglTarikTerakhir) < oP.Numeric_Value:
    #rentang penarikan masih dalam waktu BATAS_WAKTU_PENARIKAN_NORMAL (default 6 bulan (365/2 hari))
    return 0

  return 1  

def CekRentangWaktuPenarikan(config, noRekening):
  tgl_transaksi = GetLastTglPenarikan30(config, noRekening)
  if tgl_transaksi:
...
def CekSaldoIuranMin(config, noRekening):
  # cek saldo iuran
  oRekening = config.CreatePObjImplProxy('RekInvDPLK')
  oRekening.key = noRekening
