...
def remakeString(s):
  ...
  return s.lstrip().rstrip().replace('. ','_').replace('.','_').replace(' ','_').upper()
...
def NasabahDPLK(config, con):
  ...
  if res >= 0:
    ...
    #TEST CODE: KHUSUS DIBATASI UNTUK KODE_CAB_DAFTAR = '301' 
    q = con.GetQuery('select * from NASABAH_DPLK n, REKENING_DPLK r \
      where r.KODE_CAB_DAFTAR = \'301\' and r.NO_PESERTA = n.NO_PESERTA')
    
    while not q.Eof():
      o = config.CreatePObject('NasabahDPLK')
      o.no_peserta = q.GetFieldValue('NO_PESERTA')
      o.nama_lengkap = q.GetFieldValue('NAMA_LENGKAP')
      y,m,d = q.GetFieldValue('TGL_REGISTRASI')[:3]
      o.tgl_registrasi = config.ModLibUtils.EncodeDate(y,m,d)
      o.tempat_lahir = q.GetFieldValue('TEMPAT_LAHIR')
      y,m,d = q.GetFieldValue('TANGGAL_LAHIR')[:3]
      o.tanggal_lahir = config.ModLibUtils.EncodeDate(y,m,d)
...
def RekeningDPLK(config, con):
  ...
  if res >= 0:
    ...
    while not q.Eof():
      ...
      o.kode_cab_daftar = q.GetFieldValue('KODE_CAB_DAFTAR')
      o.sumber_dana = q.GetFieldValue('SUMBER_DANA')
      ...
      if q.GetFieldValue('TGL_AKSEPTASI') not in [[],None]:
        ...
        o.tgl_akseptasi = config.ModLibUtils.EncodeDate(y,m,d)
      
      o.usia_pensiun = q.GetFieldValue('USIA_PENSIUN')
      y,m,d = q.GetFieldValue('TGL_PENSIUN')[:3]
      o.tgl_pensiun = config.ModLibUtils.EncodeDate(y,m,d)
      y,m,d = q.GetFieldValue('TGL_PENSIUN_DIPERCEPAT')[:3]      
      o.tgl_pensiun_dipercepat = config.ModLibUtils.EncodeDate(y,m,d)
      ...
      if q.GetFieldValue('TGL_TUTUP') not in [[],None]:
        ...
        o.tgl_tutup = config.ModLibUtils.EncodeDate(y,m,d)
      ...
      if q.GetFieldValue('TGL_SRR_AKHIR') not in [[],None]:
        ...
        o.tgl_srr_akhir = config.ModLibUtils.EncodeDate(y,m,d)
      ...
      if q.GetFieldValue('WASIAT_UMMAT').lstrip().rstrip() == 'T':
        ...
        y,m,d = q.GetFieldValue('TGL_PENSIUN')[:3]
        oRWU.tgl_berakhir = config.ModLibUtils.EncodeDate(y,m,d)
        
        #DIDAPATKAN DARI MANA INFORMASI INI ???
        oRWU.tgl_akseptasi = config.ModLibUtils.EncodeDate(\
...
def UbahAlamat(config, con):
  ...
  if res >= 0:
    ...
    while not q.Eof():
      ...
      o.tanggal_histori = config.ModLibUtils.EncodeDate(y,m,d)
      o.no_peserta = q.GetFieldValue('NO_PESERTA')
    ...
    while not q.Eof():
      ...
      if q.GetFieldValue('TGL_LAHIR_MANFAAT') not in [None,[]]:
        ...
        if y > v_TahunValid:
          ...
          #diganti dengan tahun dummy
          o.tanggal_lahir = config.ModLibUtils.EncodeDate(v_DummyYear,m,d)
...
def BagiHasil(config, con):
  ...
  if res >= 0:
    ...
    while not q.Eof():    
      ...
      dtTglBagiHasil = config.ModLibUtils.EncodeDate(y,m,d)
  ...
  if res >= 0:
    ...
    q = con.GetQuery('select  p.ID_TRANSAKSI,\
                      ...
                      where   p.ID_TRANSAKSI = t.ID_TRANSAKSI and \
                              t.NO_PESERTA = r.NO_PESERTA and \
                              r.KODE_CAB_DAFTAR = \'301\' \
    ...
    while not q.Eof():    
      ...
      dtTglTransaksi = config.ModLibUtils.EncodeDate(y,m,d)
...
def TransaksiPremi(config, con):
  global v_IP_Server, v_TahunValid, v_DummyYear
  ...
  if res >= 0:
    ...
    while not q.Eof():    
      ...
      if y <= v_TahunValid:
        ...
        #diganti dengan tahun dummy
        dtTglTransaksi = config.ModLibUtils.EncodeDate(v_DummyYear,m,d)
        ...
        #data tanggal sudah normal: tahun lebih besar dari 1800 M
        dtTglTransaksi = config.ModLibUtils.EncodeDate(y,m,d)
...
def TransaksiDPLK(config, con):
  global v_KelipatanTransaksi, v_TahunValid, v_DummyYear, v_last_tgl_transaksi, \
  ...
  if res >= 0:
    ...
    q = con.GetQuery('select  t.ID_TRANSAKSI, \
                      ...
                      from    TRANSAKSI_DPLK t, \
                              REKENING_DPLK r \
                      where   t.KODE_JENIS_TRANSAKSI <> \'A\' and \
                              t.KODE_JENIS_TRANSAKSI <> \'B\' and \
                              t.ISDELETED is NULL and \
                              r.NO_PESERTA = t.NO_PESERTA and \
                              r.KODE_CAB_DAFTAR = \'301\' \
...
def PengalihanKeDPLKLain(config, con, oTransactionBatch, query, dtTglTransaksi):
  ...
  q = con.GetQuery('select t.NAMA_DPLK2, t.AKUMULASI_IURAN, \
                    ...
                    where  t.ID_TRANSAKSI = %d and \
                           t.NAMA_DPLK2 is not null' \
                    % (query.GetFieldValue('ID_TRANSAKSI')))
...
def TransaksiBagiHasil(config, con, oTransactionBatch, query, dtTglTransaksi):
  ...
  if not qCek.Eof():
    ...
    o.idbghasil = qCek.GetFieldValue('IDBGHASIL')
...
def PindahPaketInvestasi(config, con, oTransactionBatch, query, dtTglTransaksi):
  ...
  if not q.Eof():  
    ...
    oL.mutasi_iuran_pst = q.GetFieldValue('SALDO_IURAN')
    oL.mutasi_pengembangan = q.GetFieldValue('SALDO_PENGEMBANGAN')
    oL.mutasi_peralihan = q.GetFieldValue('SALDO_PERALIHAN')
    ...
    oN.mutasi_iuran_pst = oL.mutasi_iuran_pst + o.mutasi_iuran_pst 
    oN.mutasi_pengembangan = oL.mutasi_pengembangan + o.mutasi_pengembangan 
    oN.mutasi_peralihan = oL.mutasi_peralihan + o.mutasi_peralihan
