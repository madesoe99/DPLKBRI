def SetControlsForView(form):
  ...
  form.Caption = 'Lihat Register Pendapatan Investasi ke Piutang'
...
def SetControlsForAuth(form):
  ...
  form.GetControlByName('pButton.btnClose').Visible = 1
  form.GetControlByName('pButton.btnClose').Cancel = 1
  form.Caption = 'Otorisasi Register Pendapatan Investasi ke Piutang'
...
def FormShow(form, parameter):
  ...
  if uipTransLRInvestasi.mode == 'viewdoc':
    form.Caption = 'Lihat Transaksi Pendapatan Investasi ke Piutang'
    ...
    if uipTransLRInvestasi.mode == 'view':
      ...
      form.Caption = 'Koreksi Register Pendapatan Investasi ke Piutang'
...
def btnOKClick(sender):
  ...
  if uipTransLRInvestasi.mode == 'viewdoc':
    ...
    if uipTransLRInvestasi.mode not in ['view','auth']:
      ...
      dlg = app.ConfirmDialog('Anda yakin menyetujui Pendapatan Investasi ke Piutang ini?')
      if dlg:
        # otorisasi
        app.ExecuteScript('investasi/transaksi/pendapataninvpiutang_auth',\
...
def btnCancelClick(sender):
  ...
  if uipTransLRInvestasi.mode == 'auth':
    dlg = app.ConfirmDialog('Anda yakin membatalkan Pendapatan Investasi ke Piutang ini?')
    if dlg:
      # hapus/tolak
