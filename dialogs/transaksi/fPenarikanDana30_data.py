...
def Form_OnSetDataEx(uideflist, parameterForm):
  ...
  recTransaksi.batas_penarikan_pst = recParameter.PERSEN_PENARIKAN_NORMAL / 100 * \
    recRekening.akum_iuran_pst
  recTransaksi.batas_penarikan_tmb = recParameter.PERSEN_PENARIKAN_NORMAL / 100 * \
    recRekening.akum_iuran_tmb
    
  #set jumlah tarik
  recTransaksi.jml_tarik_iuran_pk = 0.0 
  recTransaksi.jml_tarik_iuran_pst = 0.0 
  recTransaksi.jml_tarik_iuran_tmb = 0.0 
  recTransaksi.jml_tarik = 0.0 
...
def SimpanTransaksi(config, params, returns):
  ...
  try:
    #penarikan dana sebagian (30%)
    
    oT = config.CreatePObject('PenarikanDanaNormal')
