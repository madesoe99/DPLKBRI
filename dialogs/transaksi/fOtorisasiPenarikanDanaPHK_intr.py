class fOtorisasiPenarikanDanaPHK:
  ...
  def Form_OnFormShow(self, form, parameter):
    ...
    if self.Mode == 'view':
      ...
      form.Caption = 'Lihat Detil Penarikan Dana PHK'
  #--

  def bReject_Click(self, button):
    if self.App.ConfirmDialog('Anda yakin membatalkan transaksi Penarikan Dana PHK peserta %s %s?' \
      ...
      if rec.error_status:
        ...
        button.ExitAction = 1
  #-- 
  def bApproveOnClick(self, button):
    if self.App.ConfirmDialog('Anda yakin Otorisasi transaksi Penarikan Dana PHK peserta %s %s?' \
      % (self.uipPeserta.no_peserta, self.uipPeserta.nama_lengkap)):
      # reject transaksi
      ph = self.App.CreateValues(\
        ['id_transaksi',self.uipTransaksi.ID_Transaksi],\
