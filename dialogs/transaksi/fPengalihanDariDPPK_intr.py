class fPengalihanDariDPPK:
  ...
  def bSimpanClick(self, button):
    ...
    #cek kode DPPK atau nomor DPPK lain
    ...
    if (self.uipTransaksi.mutasi_iuran_pk in (None,'') or self.uipTransaksi.mutasi_iuran_pk < self.uipParameter.PRESISI_ANGKA_FLOAT) and \
      ...
      return
    if self.app.ConfirmDialog('Anda yakin Simpan transaksi pengalihan Dana dari DPPK lain peserta %s %s?' % (self.uipPeserta.no_peserta, self.uipPeserta.nama_lengkap)):
      pass
    else:
      return 0
