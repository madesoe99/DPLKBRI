...
def bOKClick(sender):
  ...
  if uipInput.GetFieldValue('SRRCalc.ID_SRRCalc') in ['',None]:
    app.ShowMessage('SRR yang dipakai untuk Rebond masih kosong! Mohon dipilih dahulu.')
    return
  ...
  #tambahan haris
  recInfo.execFile = 'transaksi/L_Rebond'
  ...
  try:
    if app.ConfirmDialog('Anda yakin akan melakukan penghitungan REBOND '\
      'untuk semua Peserta DPLK?'):
      
      #pid = app.ExecuteScriptTrackable('transaksi/L_Rebond', ph)  
      app.ExecuteScript('longscripts/ExecuteLongScript', ph)

      #pcConsole.ConsoleFilterName = 'Rebond_%d' % (pid)
