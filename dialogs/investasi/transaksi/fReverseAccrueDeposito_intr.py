...
def btnOKClick(sender):
  ...
  if uipTransLRInvestasi.mode == 'viewdoc':
    ...
    if uipTransLRInvestasi.mode not in ['view','auth']:
      ...
      if dlg: CetakAdvis(app, uipTransLRInvestasi, uipInvestasi)
...
def btnPrintClick(sender):
  ...
  uipInvestasi = form.GetUIPartByName('uipInvestasi')
  form.CommitBuffer()
  CetakAdvis(app, uipTransLRInvestasi, uipInvestasi)

def CetakAdvis(app, uipTransLRInvestasi, uipInvestasi):
  lHeader = ['No. Bilyet', 'Rekening Deposito', 'Pihak Ketiga','No. Batch','Tgl Transaksi', 'Debet', 'Kredit', \
             'Keterangan', 'Nominal Akhir', 'Akum. Bagi Hasil', 'Piutang Bagi Hasil']
  sHeader = '|'.join(lHeader)
  tgl = uipTransLRInvestasi.tgl_transaksi
  sTgl = '%s-%s-%s' % (tgl[2],tgl[1],tgl[0])
  lData = [str(uipTransLRInvestasi.GetFieldValue('LInvestasi.no_bilyet')),\
           str(uipTransLRInvestasi.rekening_deposito),\
  ...
  sData = '|'.join(lData)
  ph = app.CreateValues(['nama_advis','Koreksi Hasil Investasi'],['sHeader',sHeader],['sData',sData],['inputer',uipTransLRInvestasi.user_id])
