...
def btnShowClick(sender):
  ...
  app = form.ClientApplication
  uipNoData = form.GetUIPartByName('uipNoData')
  qRegisterTransaksiObligasi = form.GetPanelByName('qRegisterTransaksiObligasi')
  ...
  qRegisterTransaksiObligasi.SetParameter('dateFrom',dateFrom)
  qRegisterTransaksiObligasi.SetParameter('dateUntil_tmrw',tmrw_dateUntil)

  uipNoData.isDisplay
  if uipNoData.isDisplay == 1:
    qRegisterTransaksiObligasi.Refresh()
  else:
    # uipNoData.isDisplay == 0
    qRegisterTransaksiObligasi.DisplayData()
