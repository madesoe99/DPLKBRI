...
def SetControlsForAuth(form):
  ...
  form.GetControlByName('pButton.btnClose').Cancel = 1
  form.Caption = 'Otorisasi koreksi nilai wajar'
  SetSelectorReadOnly(form)
...
def FormShow(form, parameter):
  ...
  if uipPendapatanObligasi.mode == 'viewdoc':
    form.Caption = 'Lihat Transaksi koreksi nilai wajar'
    ...
    if uipPendapatanObligasi.mode == 'view':
      ...
      SetSelectorReadOnly(form)
      form.Caption = 'Edit transaksi koreksi nilai wajar'
...
def NilaiWajar_OnExit (sender) :
  form = sender.OwnerForm
  app = form.ClientApplication
  uipPendapatanObligasi = form.GetUIPartByName('uipPendapatanObligasi')
  uipObligasi = form.GetUIPartByName('uipObligasi')
  uipPendapatanObligasi.Edit()
  uipPendapatanObligasi.Nominal_Total = uipPendapatanObligasi.Nilai_Wajar * uipObligasi.akum_nominal/100
  uipPendapatanObligasi.nominal = (uipPendapatanObligasi.Nilai_Wajar - 100) * uipObligasi.akum_nominal/100
...
def btnOKClick(sender):
  ...
  if uipPendapatanObligasi.mode == 'viewdoc':
    ...
    if uipPendapatanObligasi.mode not in ['view','auth']:
      ...
      dlg = app.ConfirmDialog('Anda yakin menyetujui koreksi nilai wajar ini?')
      if dlg:
        # otorisasi
        app.ExecuteScript('investasi/transaksi/koreksinilaiwajarobligasi_auth',\
...
def btnCancelClick(sender):
  ...
  if uipPendapatanObligasi.mode == 'auth':
    dlg = app.ConfirmDialog('Anda yakin membatalkan koreksi nilai wajar ini?')
    if dlg:
      # hapus/tolak
...
def CetakAdvis(app, uipPendapatanObligasi, uipObligasi):
  lHeader = ['Nama Investasi', 'Issuer', 'Paket Investasi', \
             'Tgl Transaksi', 'Nilai Wajar (%)', 'Nominal Total', 'Pendapatan', \
  ...
  lData = [str(uipPendapatanObligasi.GetFieldValue('LObligasi.nama_obligasi')),\
           ...
           str(sTgl),\
           app.ModLibUtils.FormatFloat(',0.00 %',uipPendapatanObligasi.nilai_wajar or 0),\
           app.ModLibUtils.FormatFloat(',0.00',uipPendapatanObligasi.nominal_total or 0.0),\
           ...
           str(sTglJT)]
  sData = '|'.join(lData)
  ph = app.CreateValues(['nama_advis','Koreksi Nilai Wajar'],['sHeader',sHeader],['sData',sData],['inputer',uipPendapatanObligasi.user_id])
