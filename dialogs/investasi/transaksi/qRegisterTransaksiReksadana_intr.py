...
def btnShowClick(sender):
  ...
  app = form.ClientApplication
  uipNoData = form.GetUIPartByName('uipNoData')
  qRegisterTransaksiReksadana = form.GetPanelByName('qRegisterTransaksiReksadana')
  ...
  qRegisterTransaksiReksadana.SetParameter('dateFrom',dateFrom)
  qRegisterTransaksiReksadana.SetParameter('dateUntil_tmrw',tmrw_dateUntil)

  uipNoData.isDisplay
  if uipNoData.isDisplay == 1:
    qRegisterTransaksiReksadana.Refresh()
  else:
    # uipNoData.isDisplay == 0
    qRegisterTransaksiReksadana.DisplayData()
