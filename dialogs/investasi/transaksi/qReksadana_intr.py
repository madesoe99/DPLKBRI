...
def btnShowClick(sender):
  ...
  if date_ltd:
    ...
    qReksadana.SetParameter('dateFrom', dateFrom)
    qReksadana.SetParameter('dateUntil_tmrw', tmrw_dateUntil)
  ...
  qReksadana.SetParameter('status_ltd', status_ltd)
  qReksadana.SetParameter('status', status)

  if uipNoData.isDisplay == 1:
    qReksadana.Refresh()
  else:
    # uipNoData.isDisplay == 0
    qReksadana.DisplayData()
