...
def btnShowClick(sender):
  ...
  if date_ltd:
    ...
    qObligasi.SetParameter('dateFrom', dateFrom)
    qObligasi.SetParameter('dateUntil_tmrw', tmrw_dateUntil)
  ...
  qObligasi.SetParameter('status_ltd', status_ltd)
  qObligasi.SetParameter('status', status)

  if uipNoData.isDisplay == 1:
    qObligasi.Refresh()
  else:
    # uipNoData.isDisplay == 0
    qObligasi.DisplayData()
