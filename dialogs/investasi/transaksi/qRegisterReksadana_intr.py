...
def btnShowClick(sender):
  ...
  app = form.ClientApplication
  uipNoData = form.GetUIPartByName('uipNoData')
  qRegisterReksadana = form.GetPanelByName('qRegisterReksadana')
  ...
  qRegisterReksadana.SetParameter('dateFrom',dateFrom)
  qRegisterReksadana.SetParameter('dateUntil_tmrw',tmrw_dateUntil)

  uipNoData.isDisplay
  if uipNoData.isDisplay == 1:
    qRegisterReksadana.Refresh()
  else:
    # uipNoData.isDisplay == 0
    qRegisterReksadana.DisplayData()
