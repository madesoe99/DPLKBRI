...
def btnShowClick(sender):
  ...
  app = form.ClientApplication
  uipNoData = form.GetUIPartByName('uipNoData')
  qRegisterSaham = form.GetPanelByName('qRegisterSaham')
  ...
  qRegisterSaham.SetParameter('dateFrom',dateFrom)
  qRegisterSaham.SetParameter('dateUntil_tmrw',tmrw_dateUntil)

  uipNoData.isDisplay
  if uipNoData.isDisplay == 1:
    qRegisterSaham.Refresh()
  else:
    # uipNoData.isDisplay == 0
    qRegisterSaham.DisplayData()
