...
def btnShowClick(sender):
  ...
  app = form.ClientApplication
  uipNoData = form.GetUIPartByName('uipNoData')
  qRegisterTransaksiDeposito = form.GetPanelByName('qRegisterTransaksiDeposito')
  ...
  qRegisterTransaksiDeposito.SetParameter('dateFrom',dateFrom)
  qRegisterTransaksiDeposito.SetParameter('dateUntil_tmrw',tmrw_dateUntil)

  uipNoData.isDisplay
  if uipNoData.isDisplay == 1:
    qRegisterTransaksiDeposito.Refresh()
  else:
    # uipNoData.isDisplay == 0
    qRegisterTransaksiDeposito.DisplayData()
