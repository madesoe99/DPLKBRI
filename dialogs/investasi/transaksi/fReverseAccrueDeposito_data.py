...
def uipInvestasiSetData(uipInvestasi):
  ...
  oDepo.Key = rec_inv.id_investasi
  ...
  rec_tpi.SetFieldByName('rekening_deposito', oDepo.rekening_deposito)
  
  sSQL = "select nominal from akumaccrue where id_investasi = %s" % oInvestasi.id_investasi
  rSQL = config.CreateSQL(sSQL).RawResult
  if not rSQL.Eof:
    accrue = rSQL.nominal or 0.0
  else:
    accrue = 0
    
  rec_tpi.SetFieldByName('accrue', accrue)
