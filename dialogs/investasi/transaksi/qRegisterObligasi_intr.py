...
def btnShowClick(sender):
  ...
  app = form.ClientApplication
  uipNoData = form.GetUIPartByName('uipNoData')
  qRegisterObligasi = form.GetPanelByName('qRegisterObligasi')
  ...
  qRegisterObligasi.SetParameter('dateFrom',dateFrom)
  qRegisterObligasi.SetParameter('dateUntil_tmrw',tmrw_dateUntil)

  uipNoData.isDisplay
  if uipNoData.isDisplay == 1:
    qRegisterObligasi.Refresh()
  else:
    # uipNoData.isDisplay == 0
    qRegisterObligasi.DisplayData()
