...
#       ds  = res.Packet.danapaket
    ...
    if lamp == 0: # lampiran 1
      ...
      #workbook.SetCellValue(19,7, 'TEST')
...
def IsiDanaPaket(ds, workbook, worksheetname):
    ...
    workbook.SetCellValue(i+19,1, n)
    workbook.ActivateWorksheet(worksheetname)
    while i < n:
      rec = ds.GetRecord(i)
      #strtglPenempatan =
      workbook.SetCellValue(i+19,2, rec.nomor_bilyet)
      workbook.SetCellValue(i+19,3, rec.tgl_penempatan)
      workbook.SetCellValue(i+19,4, rec.tgl_pencairan)
      workbook.SetCellValue(i+19,5, rec.nominal_investasi)
      workbook.SetCellValue(i+19,6, '')
