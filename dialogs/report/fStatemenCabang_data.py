...
def ConstructReportHeader(config, no_peserta, dari_tanggal, hingga_tanggal, saldo_awal, strTepiKiri, oFile):
  ...
  kota_kodepos = '%s %s %s' % (oNasabahDPLK.alamat_surat_kecamatan,oNasabahDPLK.alamat_surat_kota, oNasabahDPLK.alamat_surat_kode_pos)
  ...
  oFile.write(strTepiKiri +'%13s%s' % ('', alamat2));NextLine(config, oFile)
...
def ContructSumValues(config, no_peserta, str_dari_tanggal, str_hingga_tanggal_plus, \
  ...
  oFile.write(strTepiKiri +'%13s%-40s: %18s' % ('', 'Akumulasi Iuran Peserta', moduleapi.FormatFloatStd(config, sumIuranPst)));NextLine(config, oFile)
  oFile.write(strTepiKiri +'%13s%-40s: %18s' % ('', 'Akumulasi Iuran Pemberi Kerja', moduleapi.FormatFloatStd(config, sumIuranPK)));NextLine(config, oFile)
  oFile.write(strTepiKiri +'%13s%-40s: %18s' % ('', 'Akumulasi Penarikan Tahun '+config.FormatDateTime('yyyy',hingga_tanggal), moduleapi.FormatFloatStd(config, akumPenarikanTotal)));NextLine(config, oFile)
  oFile.write(strTepiKiri +'%13s%-40s: %18s' % ('', 'Hasil Investasi', moduleapi.FormatFloatStd(config, dictJenisTransaksi['G'])));NextLine(config, oFile)
  ...
  oFile.write(strTepiKiri +'%13s%-40s: %18s' % ('', 'Pengalihan Dari Dana Pensiun Lain', moduleapi.FormatFloatStd(config, akumPengalihanTotal)));NextLine(config, oFile)
...
def resSQLCatMutasiIndv(config, no_peserta, str_dari_tanggal, str_hingga_tanggal_plus):
  strSQL = '\
    select tgl_transaksi \
      ...
      , substring(keterangan,20,10) as keterangan \
    ...
    where tgl_transaksi >= \'%s\' \
      ...
      and isCommitted = \'T\' \
    order by id_transaksi, tgl_transaksi ' \
    % (str_dari_tanggal, str_hingga_tanggal_plus, no_peserta)
...
##  saldo 19
  ...
  while not resSQL.Eof:
    ...
    if resSQL.kode_jenis_transaksi != 'M':
      # bukan transaksi manual
      if resSQL.mutasi_total < 0.0 and resSQL.kode_jenis_transaksi in ['K']:
         transaksi = 'Koreksi %s' % (dictJenisTransaksiDPLK[resSQL.kode_jenis_transaksi])
      else:
         if resSQL.kode_jenis_transaksi in ['G']:
            transaksi = '%s %s' % (dictJenisTransaksiDPLK[resSQL.kode_jenis_transaksi], resSQL.keterangan)
         else:
    ...
    #Sementara di tutup by ade herman 2010-11-04
    #if mutasi in ['D']:
    #   oFile.write(strTepiKiri +' %s  %-30s    %12s%18s%18s' %
    #     (tanggal
    #      , transaksi
...
def resSQLNasabahDPLK(config, kode_cab_daftar, isNasabahCorporateIncl, str_hingga_tanggal):
  strFilterNasabahCorp = 'and kode_nasabah_corporate is null'
  if isNasabahCorporateIncl:
    strFilterNasabahCorp = ''

  strSQL = \
    'select n.no_peserta '\
    'from NasabahDPLK n, RekeningDPLK r '\
    'where kode_cab_daftar = \'%s\' '\
    '  %s '\
    '  and r.status_dplk = \'A\' '\
    '  and n.no_peserta = r.no_peserta '\
    '  and n.operation_code = \'T\' '\
    ...
    % (kode_cab_daftar, strFilterNasabahCorp, str_hingga_tanggal)
  return config.CreateSQL(strSQL).RawResult
...
def WriteToFile(config, parameter, returnpacket):
  ...
  kode_cab_daftar = parameter.FirstRecord.branch_code
  isNasabahCorporateIncl = parameter.FirstRecord.isNasabahCorporateIncl
  ...
  resSQL = resSQLNasabahDPLK(config, kode_cab_daftar, isNasabahCorporateIncl, str_hingga_tanggal_plus)
  resSQL.First()
  ...
  if not resSQL.Eof:
    ...
    sTextFile = 'statemen_cabang_%s' % (strTglFile)
