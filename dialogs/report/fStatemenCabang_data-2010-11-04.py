...
## end By Ade herman
  ...
  oFile.write(strTepiKiri +'-----------------------------------------------------------------------------------------------');NextLine(config, oFile)
  oFile.write(strTepiKiri +'%12s%83s' % ('SALDO AKHIR', moduleapi.FormatFloatStd(config, saldo_curr)));NextLine(config, oFile)
  oFile.write(strTepiKiri +'===============================================================================================');NextLine(config, oFile)
  # terus pindah baris hingga ke halaman berikutnya
  NextPagePeserta(config, oFile)

def resSQLNasabahDPLK(config, kode_cab_daftar, isNasabahCorporateIncl):
  ...
  strSQL = \
    ...
    % (kode_cab_daftar, strFilterNasabahCorp)
  return config.CreateSQL(strSQL).RawResult
...
def WriteToFile(config, parameter, returnpacket):
  ...
  resSQL = resSQLNasabahDPLK(config, kode_cab_daftar, isNasabahCorporateIncl)
  resSQL.First()
