...
## end By Ade Herman 2010-03-24
    ...
    if resSQL.kode_jenis_transaksi != 'M':
      ...
      if resSQL.mutasi_total < 0.0 and resSQL.kode_jenis_transaksi in ['K']:
         transaksi = '(%s) Koreksi <font size="2">%s</font>' % (resSQL.kode_jenis_transaksi, dictJenisTransaksiDPLK[resSQL.kode_jenis_transaksi])
      else:
         if resSQL.kode_jenis_transaksi in ['G']:
            transaksi = '(%s) <font size="2">%s %s</font>' % (resSQL.kode_jenis_transaksi, dictJenisTransaksiDPLK[resSQL.kode_jenis_transaksi], resSQL.keterangan)
         else:
            transaksi = '(%s) <font size="2">%s</font>' % (resSQL.kode_jenis_transaksi, dictJenisTransaksiDPLK[resSQL.kode_jenis_transaksi])
    else:
      # transaksi manual, disesuaikan dengan kode_transaksi_manual
