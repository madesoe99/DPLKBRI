def setSaldoTotal(uideflist):
  uipRekeningDPLK = uideflist.uipRekeningDPLK
  rec = uipRekeningDPLK.Dataset.GetRecord(0)

  rec.saldo_total = \
    (rec.akum_dana_iuran_pk or 0.0) \
    + (rec.akum_dana_iuran_pst or 0.0) \
    + (rec.akum_dana_pengembangan or 0.0) \
    + (rec.akum_dana_peralihan or 0.0)

def FormEndSetData(uideflist, uiname, objdata):
  config = uideflist.Config
  uipN = uideflist.uipNasabahDPLK
  recN = uipN.Dataset.GetRecord(0)
  ...
  if recN.GetFieldByName('LRekeningDPLK.status_wasiat_ummat') == 'T':
    ...
    sSQL = 'select NO_POLIS,TGL_AKSEPTASI,TGL_BERAKHIR,BESAR_PREMI, MANFAAT_ASURANSI '\
      'from REKENINGWASIATUMMAT where NO_PESERTA = \'%s\'' % (recN.no_peserta)
  ...
  if recN.GetFieldByName('LRekeningDPLK.status_anuitas') == 'T':

    #peserta auto debet, set field uipRekeningAD, pake SQL biar gampang
    sSQL = 'select NO_REKENING,NAMA_ASURANSI,NO_POLIS_ANUITAS,TGL_PEMBELIAN_ANUITAS '\
      'from REKENINGANUITAS where NO_PESERTA = \'%s\'' % (recN.no_peserta)
    rSQL = config.CreateSQL(sSQL).RawResult
