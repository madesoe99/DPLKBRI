def GetFormInfoByCIFCode(kode_jenis_registercif):
  if kode_jenis_registercif == 'A':
    ...
    return ['fRegisterPindahPaketInvestasiUnitized','RekeningNasabah']
  #elif kode_jenis_registercif == 'U':
  #  return ['fRegisterWasiatUmmat','RekeningDPLK']
...
def btnOKClick(sender):
  ...
  if classname <> 'RekeningNasabah':
    key = 'PObj:%s#NO_PESERTA=%s' % (classname, keyvalue)
  else:
    key = 'PObj:%s#NOMOR_REKENING=%s' % (classname, keyvalue)
